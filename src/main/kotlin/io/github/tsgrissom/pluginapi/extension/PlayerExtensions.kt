...
fun Player.generateTemporaryAttributesList(
    isAfk: Boolean,
    ...
    if (withHeader)
        builder
            ...
            .append(this.name).color(ChatColor.YELLOW)
            .append(" : ").color(ChatColor.DARK_GRAY)
            .append("Temporary").color(ChatColor.GRAY)
            .append("\n")

    if (!excludeGamemode) {
        val gm = this.gameMode.name.capitalizeAllCaps()
        appendPrefix()
        builder
    ...
    if (!excludeWorld) {
        val worldType: WorldType? = this.world.worldType

        appendPrefix()
        builder
            .append("World: ").color(ChatColor.GRAY)
            .append(this.world.name).color(ChatColor.YELLOW)
        if (worldType != null)
            builder
                .append(" + ").color(ChatColor.DARK_GRAY)
                .append("World Type: ").color(ChatColor.GRAY)
                .append(worldType.name.capitalizeAllCaps()).color(ChatColor.AQUA)
        builder.append("\n")
    }
    ...
    if (!excludeHealth) {
        ...
        val attrMaxHealth = this.getAttribute(Attribute.GENERIC_MAX_HEALTH)
    ...
    if (!excludeOxygen) {
        ...
        builder
            ...
            .append("${this.remainingAir}").color(ChatColor.YELLOW)
            .append("/").color(ChatColor.DARK_GRAY)
    ...
    if (!excludeSpeed) {
        val fly = "${this.flySpeed}"
        val walk = "${this.walkSpeed}"
    ...
    if (!excludeFlight) {
        val canFly = this.allowFlight.palatable(withColor=true)
        val isFlying = this.isFlying.palatable(withColor=true)
    ...
    if (!excludeSneaking || !excludeSprinting) {
        val isSneaking = this.isSneaking.palatable(withColor=true)
        val isSprinting = this.isSprinting.palatable(withColor=true)
    ...
    if (!excludeExperience) {
        ...
        builder
            ...
            .append("${this.level}").color(ChatColor.YELLOW)
            .append(" + ").color(ChatColor.DARK_GRAY)
            .append("Exp: ").color(ChatColor.GRAY)
            .append("${this.exp}").color(ChatColor.YELLOW)
            .append(" + ").color(ChatColor.DARK_GRAY)
            .append("Total Exp: ").color(ChatColor.GRAY)
            .append("${this.totalExperience}").color(ChatColor.YELLOW)
    }

    return builder.create()
}

fun Player.generatePermanentAttributesList(
    withHeader: Boolean = true,
    linePrefix: String = " - ",
    ...
    if (!excludeNames) {
        val dn = this.displayName
        val hasNoNickname = dn == this.name
    ...
    if (!excludeOperator) {
        val isOp = this.isOp.palatable(withColor=true)
        appendPrefix()
        builder
